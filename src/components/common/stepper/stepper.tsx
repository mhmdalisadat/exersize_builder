...
const Stepper = ({
  ...
  const validateStep = validateCurrentStep || (() => true);
  ...
  const handleStepClick = async (index: number) => {
    ...
    // Only trigger API requests for desktop version
    if (window.innerWidth >= 768) {
      // md breakpoint
      onStepClick(index);
    } else {
      // For mobile, just update the step without API request
      onStepClick(index);
    }
